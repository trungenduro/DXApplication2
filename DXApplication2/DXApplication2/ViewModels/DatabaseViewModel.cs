...
public partial class DatabaseViewModel : ObservableObject {
   ...
   // [ObservableProperty]
        ...
        if (args.Item is DHFOrder order)
        {
            ...
            if (args.DataChangeType == DataChangeType.Edit)
            {
                if(string.IsNullOrEmpty(order.客先名 ))
                {
					args.IsValid = false;
					//order.客先名 = args..;
					await Shell.Current.DisplayAlert("確認", "客先名が未入力", "OK");
					return;
				}
	...
	internal void ImportCsv(List<List<string>> csvData)
	{
		...
		{
			Shell.Current.DisplayAlert("Error", "オーダーが 重複してます\n", "OK").Wait();
			return;
		}
        else
        {
            var order = new DHFOrder { OrderNo = ordersName, 客先名 = csvData[0][1], Total = csvData.Count, };		
			unitOfWork.CustomersRepository.Add(order);
			pendingAction = () => Orders.Remove(order);
